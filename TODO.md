...
- **Server-generated mail header frame**: The original system likely auto-generated a COURIER header frame (showing FROM, DATE, TIME, SUBJECT, TO fields) as the first frame of each delivered message. See `historical/seq/compunet-pages-interviews/fan mails.seq` for an example of this format. The server should generate this frame at delivery time and prepend it to the recipient's stored message frames. The client does NOT send this frame — the server creates it from the metadata (subject, destinations, sender, timestamp). This saved bandwidth on the original 75 baud upload link.
