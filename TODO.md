...
- **Program upload data corruption (bit-7 stripping)**: VICE's ACIA TX emulation strips bit 7 from transmitted bytes before they reach the ip232 socket, despite the control register being configured for 8-bit word length ($1F). Confirmed via tcpser trace: program data arrives at tcpser with bit 7 already cleared; only the ATDT command letters have bit 7 set (a separate quirk). Downloads are unaffected (VICE's RX path preserves all 8 bits). Eliminating tcpser will NOT fix this. Fix requires: different VICE ACIA register values, a VICE configuration option, or a 7-bit-safe encoding on the upload path. See `docs/UPLOAD-BIT7-INVESTIGATION.md`.
