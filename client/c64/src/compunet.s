...
;
; ⚠ THIS FILE IS HAND-MAINTAINED. It is NOT regenerated.
; It began as gen_source.py output but has since been edited by hand — the
; ACIA driver, the auto-connect build, and every routine name and comment that
; describes verified behaviour exist only here. Re-running any disassembler
; over the ROM and overwriting this file would destroy all of that.
; gen_source.py writes compunet_rom.s, a separate throwaway file; keep it
; that way.
;
; The ROM half of this source still assembles to a byte-identical copy of the
; original ROM (verify with: cmp build/compunet.bin original_rom.bin over the
; ROM region, allowing for the generated version/ident fields).
; =================================================================
...
L801E:
    ...
    .byte $4F, $4E                                               ; $804F "ON" (part of *CON match at $804D)
    .include "build/ident.inc"
...
L8094:
    ...
    JMP DISC_OPEN_PROMPT
    JMP DISC_OPEN_NAMED
    JMP DISC_CLOSE_STATUS
...
L840C:
    ...
    .byte .lobyte(EDITOR_GET-1), .hibyte(EDITOR_GET-1)
    .byte .lobyte(EDITOR_PUT-1), .hibyte(EDITOR_PUT-1)
    .byte .lobyte(EDITOR_STORE-1), .hibyte(EDITOR_STORE-1)
    .byte .lobyte(L85DE-1), .hibyte(L85DE-1)
    .byte .lobyte(L899C-1), .hibyte(L899C-1)
    .byte .lobyte(L875B-1), .hibyte(L875B-1)
    .byte .lobyte(EDITOR_DOS-1), .hibyte(EDITOR_DOS-1)
L8439:
    LDX #.lobyte(L9589)
...
L84FD:
    JMP L8495
; --- EDITOR_GET ---
; Editor "GET" ($8500): load a stored file into the buffer.
; Opens the file for read ('R'/'S'), points the byte source at DISC_BYTE_IN,
; then decodes frame after frame — each [4-byte header][body][$00] — appending
; every one with PAGE_COMMIT until $C15D (the KERNAL ST latch) reports EOF.
; A stored file may therefore hold SEVERAL frames back-to-back with no count,
; no index and no container: the $00 ends a FRAME, not the file. See
; docs/spec/08-subsystems.md §8.4.4.
EDITOR_GET:
    LDX #.lobyte(L853D)
    LDY #.hibyte(L853D)
    JSR CURSOR_HOME
    LDA #$52                            ; 'R' — open for read
    LDX #$53                            ; 'S' — SEQ file
    JSR DISC_OPEN_PROMPT
    BCS L8576
L8510:
    JSR L8495
    LDA #.lobyte(DISC_BYTE_IN)                         ; workspace
    STA $C140
    LDA #.hibyte(DISC_BYTE_IN)                         ; workspace
    STA $C141
    LDX #$08                            ; KERNAL_CHKIN
    JSR $FFC6
    BCS L8537
...
L853D:
    ...
    BRK
; --- EDITOR_PUT ---
; Editor "PUT" ($8541): save the CURRENT page only.
; Opens for write, emits the flags byte (FRAME_FLAGS_BYTE) then CHROUTs the
; current page's bytes up to and including its $00 terminator. Exactly one
; frame is written, so the file has a single [header][body][$00].
EDITOR_PUT:
    LDX #.lobyte(L857C)
    LDY #.hibyte(L857C)
    JSR CURSOR_HOME
    LDA #$57                            ; 'W' — open for write
    LDX #$53                            ; 'S' — SEQ file
    JSR DISC_OPEN_PROMPT
    BCS L8576
    JSR L8CAB
...
L857C:
    ...
    BRK
; --- EDITOR_STORE ---
; Editor "STORE" ($8580): save the WHOLE buffer — every page — to one file.
; Walks the buffer from its start ($8015/$8016) to its end ($8017/$8018)
; CHROUTing every byte without special-casing terminators, so the frames land
; back-to-back. On passing each $00 it writes one more byte from
; FRAME_FLAGS_BYTE — the next frame's flags — which is why a frame boundary in
; a stored file is TWO bytes ($00 then the flags byte), not one.
; This is the writer for the multi-frame format read by EDITOR_GET.
EDITOR_STORE:
    LDX #.lobyte(L85CE)
    LDY #.hibyte(L85CE)
    JSR CURSOR_HOME
...
; --- FRAME_FLAGS_BYTE ---
; Return the flags byte to write ahead of a frame ($85D4): $00 normally,
; or $01 when bit 7 of $C156 is set. Note DISC_BYTE_IN maps $01 back to $00
; on read, so the stored flags byte always loads as $00.
FRAME_FLAGS_BYTE:
    LDA #$00
    BIT $C156
    BPL L85DD
...
; --- EDITOR_PRINT ---
; Print the current page to a printer ($85E4, ROMCALL_15 / editor "PRINT").
; Decodes the page out of the buffer (skipping its 4-byte header) and CHROUTs
; it to device 4, expanding RLE runs and dropping colour codes; reports
; "PRINTER ERROR" if the device does not respond. Nothing to do with the
; screen, despite the old name.
EDITOR_PRINT:
    STX $1D
    STY $1E
    LDA #.lobyte(L8A81)
...
; --- EDITOR_DOS ---
; Editor DOS command shell ($869E, ROMCALL_20 / editor "DOS").
; Prompts "INPUT DOS COMMANDS OR STOP" and passes each line to the drive
; command channel. No frame or file transfer happens here.
EDITOR_DOS:
    JSR L874B
    BCC L86A4
L86A3:
...
L8A8A:
    ...
    RTS
; --- DISC_BYTE_IN ---
; Byte source for reading a stored file ($8A8C). Installed in the $C140/$C141
; vector by EDITOR_GET; the frame decoder fetches every byte through here.
; ⚠ It maps $01 to $00 on EVERY byte read. A stored body therefore cannot carry
; a literal $01, and the flags byte written by FRAME_FLAGS_BYTE always loads
; back as $00. Sets $C15D from the KERNAL ST so EDITOR_GET can detect EOF.
; (These first four instructions were previously mis-decoded as raw .byte data;
; they assemble to the same bytes AE 5D C1 F0 03 A9 00 60.)
DISC_BYTE_IN:
    LDX $C15D                           ; already at EOF?
    BEQ L8A94
    LDA #$00
    RTS
L8A94:
...
; --- PAGE_COMMIT ---
; Commit the page just decoded into the editor buffer ($8ABE, ROMCALL_13).
; Copies from the current page pointer ($8019/$801A) up to the buffer end
; ($8017/$8018), re-encodes the screen into the buffer, then makes the new
; page current. Despite the old name this touches NO disc: the caller has
; already read the bytes (see the GET loop at EDITOR_GET).
PAGE_COMMIT:
    LDX $8019
    LDY $801A
    JSR L8C70                           ; SCREEN_TO_BUFFER
    LDX $8017
    LDY $8018
    JSR SCREEN_TO_BUFFER
    LDX $8017
...
; --- SCREEN_TO_BUFFER ---
; Encode the visible screen into the editor buffer ($8AEB, ROMCALL_14).
; Walks all 24 screen lines via ($D1),Y and their colour via ($F3),Y, writes
; the 4-byte frame header (L8C05) then the body, emitting colour changes and
; the $06/$07 RLE runs, and trims trailing spaces on each line.
; Despite the old name this touches NO disc — it only fills the buffer.
SCREEN_TO_BUFFER:
    STX $1D
    STY $1E
    JSR L8C05
...
; --- DISC_OPEN_PROMPT ---
; Open a disc file, prompting for the name ($9171, ROMCALL_29).
; Prints " FILE NAME? ", reads the name into $801E, appends ",S,<mode>" and
; OPENs it on device 8. A = mode ('R'/'W'), X = file type ('S'/'P').
; Used by the editor GET / PUT / STORE commands. Returns C=1 on failure.
DISC_OPEN_PROMPT:
    STA $C147
    STX $8032
    LDX #.lobyte(L9256)
...
; --- DISC_OPEN_NAMED ---
; Open a disc file whose name the caller already holds ($91B2, ROMCALL_30).
; As DISC_OPEN_PROMPT but skips the prompt: the name is already in $801E and
; Y = its length. A = mode ('R'/'W'), X = file type ('S'/'P').
; Used by the terminal's SAVE / LOAD. Returns C=1 on failure.
DISC_OPEN_NAMED:
    STA $C147
    STX $8032
    STY $19
...
; --- DISC_CLOSE_STATUS ---
; Close the disc file and report any drive error ($92CD, ROMCALL_31).
; CLOSEs channel 8, reads the drive status channel and prints "DISK ERROR "
; plus the drive's message if the status is non-zero. Returns C=1 on error.
DISC_CLOSE_STATUS:
    JSR L9290
    LDA #$08
    JSR $FFC3
...
; =================================================================
; STRIPPED PROTOCOL ENGINE
; =================================================================
; The original X.25 protocol engine (~2.3K) has been removed.
; All protocol handling is done by the ACIA driver.
; Only stubs and string constants remain for ROM compatibility.
...
; --- String: "ABORTED" (referenced by MODEM_SEND_CMD) ---
L9CF4:
    .byte $93, $0E, $C1, $42, $4F, $52, $54, $45, $44, $0D, $0D, $00

; --- String: "CONNECTING..." (referenced by ACIA_PROTO_CONNECT) ---
L9E59:
    .byte $C3, $4F, $4E, $4E, $45, $43, $54, $49, $4E, $47, $2E, $2E, $2E, $0D, $11, $00
; =================================================================
; TERMINAL CODE
; =================================================================
...
ROMCALL_29      = $8157   ; DISC_OPEN_PROMPT
ROMCALL_30      = $815A   ; DISC_OPEN_NAMED
ROMCALL_31      = $815D   ; DISC_CLOSE_STATUS
