...
| §4.3 | Response types ACK/DIR/FRAME/ERROR/LINKING | `compunet_server.py` `RESP_*`; WebSocket `_ws_wrap` prefix |
...
| Spec | Claim | Source |
|---|---|---|
| §5.1 | 40×24 content grid | `client/amiga/src/frame_control.c` (cursor bounds `0x28`/`0x17`); `server/terminal.py` (`range(24)`, 40 cols) |
...
| §5.4 | Font = C64 char ROM | `client/web/charrom.js`; Amiga `build_font` (`0x11d9c0`/`0x11ddc0`) |
| §5.5 | 16-colour palette; index→pen remap | `client/web/petscii.js` (RGB); `client/amiga/src/globals.c` `g_palette` |
...
| §6.4 | RLE `$06`/`$07`, `1+N` count | `server/terminal.py` `expand_frame`; `client/amiga/src/frame.c` `frame_rle_getchar` |
...
| Appendix | Source |
|---|---|
| §A.3 palette RGB | `client/web/petscii.js` |
| §A.5 font | `client/web/charrom.js` (matches Amiga embedded ROM) |
